...
- **High-volume sequential `fetch` in one session can trip YouTube's bot check.** Running the Lamy monthly pipeline (2026-09) right after Luna's densify batch — a lot of back-to-back `yt-dlp` invocations in one day — got almost every fetch rejected with `Sign in to confirm you're not a bot`, cookies notwithstanding (191/198 attempts failed this way; only 5 new fetches got through). This is a different failure mode from privated videos: it's the whole batch blocked at once, near-immediately, not scattered individual 404s. If a `fetch`/`densify` batch fails almost entirely with that exact message, stop — don't retry immediately, it likely won't help and may extend any block. Re-export cookies (`yt-dlp --cookies-from-browser 'chromium:Profile 1' --cookies data/youtube.cookies.txt --skip-download https://www.youtube.com`) and/or space large fetch batches out across sessions/days rather than running them back-to-back.
