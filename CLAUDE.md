...
- **Fetch pins `youtube:player_client=web` (changed from `android` 2026-09) and needs `deno` on PATH** (flake provides it; yt-dlp uses it for EJS/n-challenge). `android` and `android_vr` are skipped by yt-dlp entirely whenever `--cookies` is passed ("does not support cookies") — pairing either with our cookies file silently drops the cookies and the request still hits YouTube's bot-check; this looked like an IP block at first but was actually this client/cookie incompatibility (confirmed by testing `web`/`mweb`/`web_safari`/`web_embedded` all working cleanly with the same fresh cookies, same video, right after `android`/`android_vr` failed). `tv` still fails with "The page needs to be reloaded". `mweb` also works with cookies now (kept as documented fallback) — the earlier "needs a GVS PO token, dies image-only" note was from before we had valid cookies. Per-video yt-dlp failures are skipped and logged — a gap in `data/audio`, never an aborted batch.
